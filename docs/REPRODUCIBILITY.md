...
- 저장소만 있으면 전체 회귀 테스트와 결정·증거 추적성을 확인할 수 있습니다.
- 실제 이상 맵을 다시 만들려면 라이선스가 있는 데이터셋, 모델 가중치, 체크포인트와 GPU 환경이 필요합니다.
- 재실행 결과는 입력·모델·원시 맵·평가 결과를 SHA-256으로 연결하며, 원본 이미지와 로컬 경로는 공개 증거에 복사하지 않습니다.
...
정확한 체크포인트·교사 가중치 인자는 [EfficientAD 재현 절차](SHEET_METAL_EVALUATION.md#재현-방법)에 있습니다.
