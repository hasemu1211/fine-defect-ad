...
- 256×256로 전체 이미지를 축소한 EfficientAD 기준선은 Image AU-ROC `0.595370`, AU-PRO@0.05 `0.020582`였습니다.
- 같은 체크포인트에 E2-Split 고해상도 추론을 적용하면 각각 `0.734722`, `0.132685`로 높아졌습니다.
- TensorRT/Triton 변환 후에는 `0.733333`, `0.132769`로 근접하게 유지돼 실행 백엔드 변경의 수치 보존을 확인했습니다.
- SuperADD / DINOv3는 `0.839352`, `0.431407`을 기록했습니다. 동일 이미지 분석에서는 이 차이가 모든 결함에 균일하지 않고 **중간 크기와 낮은 compactness의 결함에서 크게 관측**됐습니다.
...
- TESTpub 사용 범위: 고정 원시 점수 평가 전용
- 추적 방법: 익명 이미지 ID, 원본·정답 마스크·이상 맵 SHA-256 결속

아래 결과는 같은 시험 입력에 대한 **고정된 사후 평가**입니다. 목적은 추론 경로와 특징 방식에 따른 결과 차이를 기록하는 것입니다.
...
E2-Split은 모델을 다시 학습하지 않고 입력의 공간 정보를 보존하는 추론 구조로 기준선보다 높은 두 지표를 기록했습니다. TensorRT/Triton 변환은 실행 최적화이며, 변환 전후의 지표 차이는 성능 보존 확인에 사용했습니다. SuperADD 결과는 같은 데이터와 평가기에서 재현한 독립 파이프라인 결과입니다.
...
결함 면적 중간 구간에서 SuperADD−E2의 평균 pixel AU-ROC 차이가 `+0.206`으로 가장 크게 관측됐습니다. 이 값은 오류 패턴을 설명하는 기술 통계입니다. 구조적 원인 검증에는 특징 표현을 통제한 별도 실험이 필요합니다. 실제 익명 이상 맵·정답 마스크 패널과 계산법은 [paired raw-map 분석](PAIRED_RAWMAP_ANALYSIS.md)에 있습니다.

## 지연 수치를 직접 비교하지 않는 이유
