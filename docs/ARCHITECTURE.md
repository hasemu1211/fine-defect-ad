...
- **EfficientAD E2-Split**은 작은 결함의 공간 정보를 보존하기 위해 타일 단위 국소 맵과 전체 장면의 전역 맵을 결합합니다.
- **SuperADD / DINOv3**는 정상 특징 메모리 뱅크와 입력 특징의 거리로 이상 맵을 만드는 별도 방식입니다.
- 두 경로는 같은 528×2112 이상 맵과 같은 평가기에서 만나므로 모델 구조가 달라도 결과를 같은 조건으로 검토할 수 있습니다.
- TensorRT/Triton 실행 경로는 EfficientAD에만 구현했습니다. SuperADD 결과는 Python 직접 추론 경로의 검증 결과입니다.
