...
def training_svg(metrics: list[dict]) -> str:
    ...
    return _svg(w,h, f'<text x="28" y="30" class="t">EfficientAD-S training loss</text><text x="28" y="50" class="s">run evidence · {len(rows)} epoch snapshots · step {mx}</text><line x1="{left}" y1="48" x2="{left}" y2="{bottom}" stroke="#cbd5e1"/><line x1="{left}" y1="{bottom}" x2="696" y2="{bottom}" stroke="#cbd5e1"/><polyline points="{points}" fill="none" stroke="#2563eb" stroke-width="3"/><text x="{left}" y="270" class="n">0</text><text x="650" y="270" class="n">{mx}</text><text x="{left}" y="72" class="n">loss {hi:.3f}</text><text x="{left}" y="238" class="n">loss {lo:.3f}</text>')


def geometry_svg(freeze: dict) -> str:
    rev = freeze['revision']; selection = freeze['selection']; e1=selection['measured_gates']['E1']; e2=selection['measured_gates']['E2']
    return _svg(720,260, f'<text x="28" y="30" class="t">Geometry selection: E1 retained</text><text x="28" y="52" class="s">validation-only paired gate · decision {escape(freeze["decision_id"])}</text><rect x="42" y="82" width="260" height="116" rx="10" fill="#dcfce7" stroke="#16a34a"/><text x="62" y="112" class="t">E1 — selected</text><text x="62" y="140" class="s">coverage / integrity / seam: pass</text><text x="62" y="164" class="s">selected={escape(selection["selected"])}</text><rect x="414" y="82" width="260" height="116" rx="10" fill="#fee2e2" stroke="#dc2626"/><text x="434" y="112" class="t">E2 — rejected</text><text x="434" y="140" class="s">revision border {rev["initial_border"]} → {rev["revised_border"]}</text><text x="434" y="164" class="s">seam gate: {str(e2["origin_seam_valid"]).lower()}</text><path d="M322 140h70" stroke="#64748b" stroke-width="2" marker-end="url(#a)"/><defs><marker id="a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L6,3z" fill="#64748b"/></marker></defs><text x="28" y="228" class="n">E2 origin/seam failure; E1 retained without test/OOD tuning.</text>')


def architecture_svg() -> str:
    boxes=[(30,'Validation good images'),(220,'EfficientAD-S\npretrained inference'),(440,'Raw anomaly maps\nSHA-256 lineage'),(640,'Validation-only\nthreshold artifact')]
    body='<text x="28" y="30" class="t">Anomaly-detection evaluation pipeline</text><text x="28" y="51" class="s">reproducible artifacts; verdict/F1 remains blocked without comparator provenance</text>'
