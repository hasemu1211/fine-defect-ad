...
def _validation_identities(identity: Mapping[str, Any]) -> tuple[tuple[str, str], ...]:
    ...
    for row in rows:
        ...
        if not isinstance(path, str) or not path.startswith("validation/good/") or not isinstance(digest, str):
            raise ValueError("validation identity must be validation/good only")
