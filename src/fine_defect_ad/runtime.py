...
def discover_docker_storage() -> dict:
    ...
    return {'status':READY, 'root':root, 'driver':driver, 'driver_type':status.get('driver-type'),
...
def _host_tensor_smoke() -> dict:
    ...
    if not python.is_file(): return {'status':'PENDING', 'reason':'HOST_TORCH_VENV_UNAVAILABLE'}
    result = _run([str(python), '-c', 'import torch; assert torch.cuda.is_available(); print(torch.cuda.mem_get_info()[1])'])
