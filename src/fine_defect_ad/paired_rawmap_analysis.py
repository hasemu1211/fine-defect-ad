...
def _outputs(root:Path,run_id:str,result:dict[str,Any],*,admit:Callable[...,Any]=preflight,writer:Callable[...,Any]=atomic_write)->dict[str,Path]:
    ...
    st=rec['dataset_relative_quantile_strata']['area_fraction']['strata']; vals=[x['mean_paired_pixel_auroc_delta'] or 0 for x in st]; names=('낮음','중간','높음')
    bars=''.join(f'<rect x="{120+i*160}" y="{205-max(0,v)*400:.1f}" width="80" height="{abs(v)*400:.1f}" fill="{'#2878b5' if v>=0 else '#c94c4c'}"/><text x="{105+i*160}" y="235" font-size="18">{names[i]} n={st[i]["count"]}</text><text x="{120+i*160}" y="260" font-size="18">{v:+.3f}</text>' for i,v in enumerate(vals))
    svg=(f'<svg xmlns="http://www.w3.org/2000/svg" width="640" height="320" viewBox="0 0 640 320"><rect width="100%" height="100%" fill="white"/><text x="20" y="36" font-size="24">동일 이미지의 픽셀 AU-ROC 차이</text><text x="20" y="62" font-size="18">정답 결함 면적 구간 · 기술 통계</text><text x="20" y="88" font-size="18">SuperADD − E2-Split · 같은 TESTpub 불량 90장</text><line x1="55" y1="205" x2="610" y2="205" stroke="#222"/>{bars}<text x="20" y="282" font-size="18">추론·재학습·임계값 조정 없이 고정 이상 맵만 분석</text><text x="20" y="307" font-size="18">파랑: 양수 · 빨강: 음수 · 모델 선택 근거 아님</text></svg>').encode()
